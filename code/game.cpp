...
int load_man_pngs(void *arg)
{
    ...
    cur_game.man_sprite_position[0].w = 210;
    for (int i = 1; i < 28; i++)
    {
        cur_game.man_sprite_position[i].h = 350;
        cur_game.man_sprite_position[i].w = 210;
        cur_game.man_sprite_position[i].x = cur_game.man_sprite_position[i-1].x+ cur_game.man_sprite_position[i].w;
...
MENU:
    ...
    cur_game.man.position_in_screen = { SCREEN_WIDTH / 2 - 75, SCREEN_HEIGHT / 2 - 50,MAN_HEIGHT,MAN_WIDTH };
    ...
    while (game_running)
    {
        ...
        //------COIN TOUCH TEST----//
        if (SDL_HasIntersectionF(&(lines[startPos + 5].getCoinBounds()),&cur_game.man.position_in_screen))
        {
            coin_collected++;
        }
